...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const queryText = groupId
      ? `SELECT * FROM asset_vehicles WHERE user_id = $1 OR group_id = $2 OR user_id IN (SELECT user_id FROM group_members WHERE group_id = $2)`
      : `SELECT * FROM asset_vehicles WHERE user_id = $1`;
    ...
    throw createError({
      statusCode: 500,
      statusMessage: "Failed to load asset vehicle records.",
    });
  } finally {
