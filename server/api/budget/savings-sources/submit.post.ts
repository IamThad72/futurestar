...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const created = await client.query(
      `INSERT INTO savings_sources (user_id, group_id, name)
       VALUES ($1, $2, $3)
    ...
    console.error("Failed to submit savings source", error);
    throw createError({
      statusCode: 500,
      statusMessage: error?.message || "Failed to submit savings source.",
    });
  } finally {
