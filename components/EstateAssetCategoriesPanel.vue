<template>
  ...
  >
    ...
    <div
      ...
      :class="[
        'estate-records-panel rounded-lg border border-base-200 bg-base-300 overflow-hidden flex flex-col',
        layout === 'modal' ? 'estate-records-panel--modal' : '',
      ]"
    >
      <h2 class="border-b border-base-200 bg-base-200/50 px-4 py-3 text-sm font-semibold shrink-0">
        {{ selectedCategory }} Records
      </h2>
      <div v-if="recordsLoading" class="p-4 text-sm text-base-content/70 flex-1 min-h-0">
        Loading records...
      </div>
      <div v-else-if="recordsLoadError" class="p-4 text-sm text-error flex-1 min-h-0">
        {{ recordsLoadError }}
      </div>
      <div v-else class="estate-records-scroll flex-1 min-h-0 flex flex-col">
        <div v-if="selectedCategory === 'Asset'" class="estate-records-cards">
          <div
            ...
            :key="r.ai_id"
            class="estate-record-card rounded-lg border border-base-200 bg-base-100 p-3 space-y-1.5 text-sm"
          >
            ...
            <div class="text-base-content/80">
              ...
              {{ r.asset_classification ?? r.classification_type ?? "—" }}
            </div>
            <div v-if="r.description" class="text-base-content/70 truncate">
              <span class="font-medium">Description:</span> {{ r.description }}
            </div>
            <div v-if="r.location" class="text-base-content/70">
              <span class="font-medium">Location:</span> {{ r.location }}
            </div>
          </div>
        ...
        <div v-else-if="selectedCategory === 'Vehicle'" class="estate-records-cards">
          ...
          >
            ...
            <div v-if="r.vin" class="text-base-content/70 text-xs">
              <span class="font-medium">VIN:</span> {{ r.vin }}
            </div>
            <div class="text-base-content/80">
              <span class="font-medium">Trust:</span> {{ r.trust_designated ? "Y" : "N" }}
            </div>
          </div>
          <p v-if="!categoryRecords.length" class="text-base-content/60 text-sm py-2">No records</p>
        </div>
        <div
          v-else-if="['Cash', 'Investment', 'Investments'].includes(selectedCategory)"
          class="estate-records-cards"
        >
          <div
            v-for="(r, i) in categoryRecords"
            :key="'cai-' + i"
            class="estate-record-card rounded-lg border border-base-200 bg-base-100 p-3 space-y-1.5 text-sm"
          >
            <div class="flex justify-between items-start gap-2">
            ...
            <div class="text-base-content/80">
              ...
              <span class="capitalize">{{ r.asset_category }}</span>
            </div>
            <div class="text-base-content/80">
              <span class="font-medium">Type:</span> <span class="capitalize">{{ r.acct_type }}</span>
            </div>
            <div v-if="r.acct_number" class="text-base-content/70 text-xs">
              <span class="font-medium">Account #:</span> {{ r.acct_number }}
            </div>
            <div class="text-base-content/80">
              <span class="font-medium">Trust:</span> {{ r.trust_designated ? "Y" : "N" }}
            </div>
        ...
        <div v-else-if="isDebtCategory" class="estate-records-cards">
          ...
          >
            ...
            <div class="text-base-content/80">
              <span class="font-medium">Type:</span> <span class="capitalize">{{ r.loan_type }}</span>
            </div>
            <div v-if="getLinkedAssetLabel(r)" class="text-base-content/70">
              <span class="font-medium">Linked:</span> {{ getLinkedAssetLabel(r) }}
            </div>
            <div v-if="r.borrower" class="text-base-content/70">
              <span class="font-medium">Borrower:</span> {{ r.borrower }}
            </div>
          </div>
          <p v-if="!categoryRecords.length" class="text-base-content/60 text-sm py-2">No records</p>
        </div>
          ...
          >
            <div class="flex justify-between items-start gap-2">
              <span class="font-semibold">
                {{ [r.number, r.street].filter(Boolean).join(" ") || "—" }}, {{ r.city }}
              </span>
              <span class="font-medium shrink-0">{{ formatMoney(r.value) }}</span>
            </div>
            <div class="text-base-content/80">
              <span class="font-medium">State:</span> {{ r.state }} <span class="font-medium">Zip:</span>
              {{ r.zipcode }}
            </div>
            <div class="text-base-content/80">
              <span class="font-medium">Trust:</span> {{ r.trust_designated ? "Y" : "N" }}
        ...
        <div v-else-if="selectedCategory === 'Insurance'" class="estate-records-cards">
          ...
          >
            ...
            <div class="text-base-content/80">
              <span class="font-medium">Intent:</span> <span class="capitalize">{{ r.intent }}</span>
            </div>
          </div>
          <p v-if="!categoryRecords.length" class="text-base-content/60 text-sm py-2">No records</p>
        </div>
...
defineProps({
  layout: {
    type: String,
    default: "page",
    validator: (v) => ["page", "modal"].includes(v),
  },
});

const emit = defineEmits(["records-updated"]);
...
const VIN_DUPLICATE_MESSAGE = "This vehicle already exists in the database. Please try again.";
...
function getLinkedAssetLabel(r) {
  ...
  const a = linkedAssets.value.find(
    (x) => x.type === r.linked_asset_type && x.id === Number(r.linked_asset_id),
  );
  return a ? a.label : "—";
}
...
const canSubmit = computed(() => {
  ...
  if (selectedCategory.value === "Asset") {
    return Boolean(
      assetClassification.value && form.title && form.description && form.value !== "",
    );
  }
...
const loadVehicleModels = async () => {
  ...
  try {
    ...
    vehicleModels.value = results.map((item) => item.Model_Name || item.Model_Name?.trim()).filter(Boolean);
  } catch {
    vehicleModels.value = [];
  } finally {
...
const submitForm = async () => {
  ...
  try {
    ...
    if (isDebtCategory.value) {
      const [linkedType, linkedId] = debtForm.linked_asset
        ? debtForm.linked_asset.split(":")
        : [null, null];
    ...
    await $fetch("/api/estate-mgmt/submit", {
      method: "POST",
      body: {
        classification_type: isAssetCategory.value ? assetClassification.value : selectedCategory.value,
        title: form.title,
